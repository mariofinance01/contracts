...
contract Mario is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 MARIO-WETH LP -> MARIO pool
    // uint256 public constant INITIAL_MARIO_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the MARIO ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("MARIO", "MARIO") {
        // Mints 1 MARIO to contract creator for initial pool setup
        ...
        feeCollectorAddress = _taxCollectorAddress;
    }
    ...
    function _getMarioPrice() internal view returns (uint256 _marioPrice) {
        try IOracle(marioOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Mario: failed to fetch MARIO price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _marioPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_marioPrice >= taxTiersTwaps[tierId]) {
    ...
    function setMarioOracle(address _marioOracle) public onlyOperatorOrTaxOffice {
        require(_marioOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints MARIO to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of MARIO to mint to
    ...
    function transferFrom(
        ...
        if (recipient == pair && takeFee) {
            uint256 sellFeeAmount = amount.div(10000).mul(sellFee);
            _transfer(sender, feeCollectorAddress, sellFeeAmount);
        ...
        if (autoCalculateTax) {
            uint256 currentMarioPrice = _getMarioPrice();
            currentTaxRate = _updateTaxRate(currentMarioPrice);
            if (currentMarioPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_marioPool != address(0), "!_marioPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_marioPool, INITIAL_MARIO_POOL_DISTRIBUTION);
    ...
    function setPairAddress(address _pairAddress) external onlyOperator {
        require(_pairAddress != address(0), "zero");
        pair = _pairAddress;
    }
    ...
    function setSellFeePercentage(uint256 _value) external {
        ...
        require(_value <= 2000, "much");
        sellFee = _value;
    }

    function setTakeFee(bool _value) external {
        require(msg.sender == feeCollectorAddress, "!fee");
        takeFee = _value;
    }
}